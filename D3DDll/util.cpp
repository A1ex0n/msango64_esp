...
//全局控制
extern "C" GameVariables g_GameVars = { false, false, true, 0 , false};
extern "C" vec2 g_HeroWorldPoint = { 0 };
std::string g_EnemyHeroName[5] = { "null","null", "null", "null", "null" };
//计算坐标（x64 下使用 64 位地址保存对象指针）
extern "C" DWORD64 g_CurrrentObject = NULL;
extern "C" DWORD64 g_SelfHeroObject = NULL;

// Home positions (used by MASM code; must be C linkage / undecorated names)
extern "C" vec2 g_homewu = { -6937.0f, -7037.0f };
extern "C" vec2 g_homewei = { 6555.0f, 6333.0f };
...
//搜索特征码
 ...
 DWORD64 ScanPattern(const char* markCode, DWORD64 nOffset, LPCSTR handle)
{
    DWORD64 StartAddr = (DWORD64)GetModuleHandleA(handle);
    DWORD64 unMoudlelenth = (DWORD64)GetModuleLen((HMODULE)StartAddr);
    ...
    /////////////////////////查找特征码/////////////////////
    int nCmp;
    for (DWORD64 i = 0; i < unMoudlelenth; i++)
    {
        nCmp = 0;
        if (m_code[0] == pScanf[i])//与第一个字节相同，首字节肯定不是通配符
        {
            ...
            {
                // 找到特征码处理
                DWORD64 ullRet = (DWORD64)(pScanf + i) + (DWORD64)nOffset;
                return ullRet;
            }
        }
    }
...
*/
//获取xy坐标
// (definitions moved above with C linkage)

// ASM 文件中实现的跟随/回城函数（x64）
extern "C" void __stdcall followEnemyAsm(DWORD* pPetID, DWORD dwEnemyID, unsigned __int64 uECX, unsigned __int64 pPetMove);
extern "C" void __stdcall gohomeAsm(DWORD* pPetID, unsigned __int64 uECX, unsigned __int64 pPetMove);

void gohome(DWORD *pPetID)
{
    unsigned __int64 uECX = msango::CGame::getInstance().m_addr[msango::CommonECX];
    unsigned __int64 pPetMove = msango::CGame::getInstance().m_addr[msango::PetMove];
    gohomeAsm(pPetID, uECX, pPetMove);
}
...
void GetPointByHeroID()
{
   ...
   /*       dump [[[[[[qword_147155070]+0C948h]+0x30]+0x10+5d*8]+0x20]
            [gamebase] + 0C948h] + 0x30] + ID * 8 + 0x10] + 0x20     单位对象
            [gamebase] + 0C948h] + 0x30] + ID * 8 + 0x10] + 0x20] + 0x18     单位对象ID
            [gamebase] + 0C948h] + 0x30] + ID * 8 + 0x10] + 0x20] + 0x78     单位对象属性
            [gamebase] + 0C948h] + 0x30] + ID * 8 + 0x10] + 0x20] + 0x78] + 0xb8     单位对象name*/

            /*
                更新偏移从这里找，引用hookpetmove所在函数的地方有个函数名，就是单位对象name的偏移
                v15 = sub_1446953E0(qword_147155070[1288].name, v32[0], 1i64, 0i64);
                v15就是单位对象
                v15+0x18 就是单位对象ID
                v15+0x78 就是单位对象属性
                [v15+0x78]+0xb8 就是单位对象name
             */
            for (size_t i = 0; i < 5; i++)
            {
                HeroID[i] = 0;
                g_EnemyHeroName[i] = "NULL";
                dwObj = memGet.RVM<DWORD64>(dwBaseAddress + 0xC948);//当前对局全体对象数组，以ID排列，ID自增
            ...
            *pPetID = dwPetID;
            ...
            {
                MOutputDebugStringExA("[MSG] err ,petid:%x ,selfid:%x", dwPetID, dwSelfID);
...
HotKeyInfo hotkeys[] = {
    ...
    {201, MOD_ALT | MOD_NOREPEAT, VK_NUMPAD2},//小键盘2 向左
    {202, MOD_ALT | MOD_NOREPEAT, VK_NUMPAD4},//小键盘4 向上
    {203, MOD_ALT | MOD_NOREPEAT, VK_NUMPAD5},//小键盘5 向下
    {204, MOD_ALT | MOD_NOREPEAT, VK_NUMPAD7},//小键盘7 三倍向右
    {205, MOD_ALT | MOD_NOREPEAT, VK_NUMPAD8},//小键盘8 三倍向左
